...
4. 저장 후 **DNS 전파**를 기다립니다 (보통 수 분 ~ 최대 24~48시간).
5. Vercel **Domains** 화면에서 상태가 **Valid / Active**로 바뀌면 완료.
...
- [ ] Vercel 가입 후 `big-choi/portfolio` 임포트 & 배포 (`*.vercel.app` 확인)
- [ ] Vercel Settings → Domains에 `boseongchoi.com`, `www.boseongchoi.com` 추가
- [ ] 가비아 DNS 관리툴에서 A / CNAME 레코드 추가 (Vercel 안내값 우선)
- [ ] `https://boseongchoi.com` 접속 & 자물쇠(HTTPS) 확인
